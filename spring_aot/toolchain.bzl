...
def _spring_aot_toolchain_impl(ctx):
    ...
    template_variables = platform_common.TemplateVariableInfo({
        "SPRING_AOT_BIN": target_tool_path,
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    springaotinfo = SpringAotInfo(
...
spring_aot_toolchain = rule(
    implementation = _spring_aot_toolchain_impl,
    attrs = {
    ...
    doc = """Defines a spring_aot compiler/runtime toolchain.
